...
public sealed class TupleType(List<Type> elementTypes) : NativelyIndexableType
{
    ...
    public override ObjectIndexer? Indexer { get; internal set; } =
        new ObjectIndexer(false, PrimitiveType.Number, TypeSimplifier.Simplify(new UnionType(elementTypes)));

    public override List<ObjectProperty> Properties { get; } = [new ObjectProperty(false, "length", new LiteralType((long)elementTypes.Count))];
    ...
    public override bool Equals(Type? other) =>
        GuardedEquals(this, other, () => other is TupleType tuple && ListEquals(ElementTypes, tuple.ElementTypes));
